...
__all__ = ['SparseMaxSGD']


OLD_PYTORCH = '1.12' in torch.__version__


class SparseMaxSGD(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
        ...
        super(SparseMaxSGD, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(SparseMaxSGD, self).__setstate__(state)
        for group in self.param_groups:
            ...
            for p, dim_p in zip(group['params'], dim_group):
                if m_row_id < (dim_p_total + dim_p):
                    ...
                    if dim <= self.small_rram_size and self.is_small_by_pos:
                        ...
                        p.data.view(p.data.size()[0], -1).narrow(1, m_row_id_p, 1).add_(-group['lr'], d_p.narrow(1, m_row_id, 1))

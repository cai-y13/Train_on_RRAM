...
__all__ = ['SparseMaxAccSGD']


OLD_PYTORCH = '1.12' in torch.__version__


class SparseMaxAccSGD(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
                 ...
                 fixed_bits={'input':8, 'weight':8},
                 fixed_scale={'input':1, 'weight':0.5}):
        defaults = dict(lr=lr, momentum=momentum, dampening=dampening,
                        weight_decay=weight_decay, nesterov=nesterov)
        if nesterov and (momentum <= 0 or dampening != 0):
            raise ValueError("Nesterov momentum requires a momentum and zero dampening")
        super(SparseMaxAccSGD, self).__init__(params, defaults)
        ...
        self.fixed_step = fixed_scale['weight'] / (2 ** fixed_bits['weight'])
    ...
    def __setstate__(self, state):
        super(SparseMaxAccSGD, self).__setstate__(state)
        for group in self.param_groups:
    ...
    def step(self, closure=None):
        ...
        for group_id, group in enumerate(self.param_groups):
            ...
            if OLD_PYTORCH:
                ...
                m_row_id = m_row_id.item() #m_row_id[0]
            ...
            for p, dim_p in zip(group['params'], dim_group):
                if m_row_id < (dim_p_total + dim_p):
                    ...
                    if dim <= self.small_rram_size and self.is_small_by_pos:
                        ...
                        p.data.view(p.data.size()[0], -1)[m_col_id, m_row_id_p] += (-group['lr'] * d_p[m_col_id, m_row_id])
                        ...
                        delta = -group['lr'] * d_p.narrow(1, m_row_id, 1).clone()
                        #update_step = delta.clone()
                        #print(update_step)
                        update_step = torch.round(torch.div(delta, self.fixed_step)) * self.fixed_step
                        #update_step.div_(self.fixed_step).int().float() * self.fixed_step
                        #update_step = delta #torch.round(delta.div_(self.fixed_step)).float() * self.fixed_step
                        p.data.view(p.data.size()[0], -1).narrow(1, m_row_id_p, 1).add_(update_step) #(-group['lr'], d_p.narrow(1, m_row_id, 1))
